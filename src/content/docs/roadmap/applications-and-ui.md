...
- An application surface also has one bounded pointer-capture slot: after a press enters an embedded game, move and release keep routing to that game outside its view, with logical coordinates clamped to its edge. Detach and destroy clear the capture.
- Keyboard focus policy remains with SwiftUI, GTK, Qt, WinUI, or Kotlin rather than the renderer. An adapter records its selected embedded game in the generation-checked surface registry; routed keys then carry opaque key codes, phases, and modifiers, and focus is cleared automatically when that view detaches or is destroyed.
