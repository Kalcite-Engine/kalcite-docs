...
`scene-check` validates one `.kscn` scene. `asset-png` converts a PNG to a `.ksp` asset. `package-lock`, `package-add`, `package-remove`, and `package-sync` manage the current project package metadata. `libs` shows known bundled libraries.
